...
const isAbsoluteLocalPath = (value: string): boolean =>
  WINDOWS_ABSOLUTE_PATH_PATTERN.test(value) || value.startsWith("/") || value.startsWith("\\\\");

export const resolveLocalAssetReference = (directory: string, value: string | undefined): string | undefined => {
  const trimmed = value?.trim();
  if (!trimmed) return undefined;
  if (URI_SCHEME_PATTERN.test(trimmed) && !WINDOWS_ABSOLUTE_PATH_PATTERN.test(trimmed)) return undefined;
  if (isAbsoluteLocalPath(trimmed)) return trimmed;

  const segments = trimmed.split(/[\\/]+/u).filter((segment) => segment.length > 0);
  return segments.length > 0 ? join(directory, ...segments) : undefined;
};
